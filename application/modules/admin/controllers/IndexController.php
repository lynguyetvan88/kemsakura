...
class Admin_IndexController extends Zend_Controller_Action {
    ...
    public function cauhinhAction() {
        ...
        if ($this->_request->isPost()) {
            ...
            $conf = HTMLPurifier_Config::createDefault();
            $purifier = new HTMLPurifier($conf);
            $title1 = $purifier->purify($this->_request->getParam('tieude'));
            $dis1 = $purifier->purify($this->_request->getParam('dis'));
            $key1 = $purifier->purify($this->_request->getParam('key'));
            $banner = stripslashes(($this->_request->getParam('banner')));
            $footer = stripslashes(($this->_request->getParam('footer')));

            $system->update_System($title1, $dis1, $key1, $banner, $footer);
        }
    ...
    public function contactAction() {
        ...
        if ($this->_request->isPost()) {
            ...
            $content = stripslashes($purifier->purify($this->_request->getParam('content')));
