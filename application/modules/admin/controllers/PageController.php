    ...
    function  editpageAction()
    {
        ...
        if ($this->_request->isPost()) 
        {
            ...
            $id=  $this->_request->getParam('id');
              $dis = $purifier->purify($this->_request->getParam('dis'));
            $key = $purifier->purify($this->_request->getParam('key'));
            $system->update_Page($title, $cat_page, $position, $active, $content,$id,$key,$dis);
        }
         $id=  $this->_request->getParam('id');
...
//    them trang moi
      ...
      function addpagesAction()
    {
        ...
        if ($this->_request->isPost()) 
        {
            ...
            $add->insert_page($menu, $title, $dis, $key, $description, $img, $position, $active, $content, $home,$price,$price_root);
    ...
    function listpagesAction()
    {
                ...
                if ($this->_request->isPost()) 
                {
                    ...
                    if(empty($cat_page_id) && !empty($title)){$data =" where title like '%$title%'";
                        $paginator = Zend_Paginator::factory($muser->list_Pages($data));}
                     if(!empty($cat_page_id) && !empty($title)){
                ...
                else{ $paginator = Zend_Paginator::factory($muser->list_Pages());}
     ...
     function  editpagesAction()
    {
        ...
        if ($this->_request->isPost()) 
        {
            ...
            $id=  $this->_request->getParam('id');
            $system->update_Pages($menu,$title, $dis, $key, $description, $images123,  $position, $active, $content, $home,$price,$price_root,$id);
        }
         $id=  $this->_request->getParam('id');
